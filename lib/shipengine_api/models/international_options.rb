...
module ShipEngineApi
  ...
  class InternationalOptions
    ...
    def valid?
      contents_validator = EnumAttributeValidator.new('String', ["merchandise", "documents", "gift", "returned_goods", "sample"])
      return false unless contents_validator.valid?(@contents)
      non_delivery_validator = EnumAttributeValidator.new('String', ["return_to_sender", "treat_as_abandoned"])
      return false unless non_delivery_validator.valid?(@non_delivery)
      return true
    end
    ...
    def contents=(contents)
      validator = EnumAttributeValidator.new('String', ["merchandise", "documents", "gift", "returned_goods", "sample"])
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] non_delivery Object to be assigned
    def non_delivery=(non_delivery)
      validator = EnumAttributeValidator.new('String', ["return_to_sender", "treat_as_abandoned"])
      unless validator.valid?(non_delivery)
        fail ArgumentError, "invalid value for 'non_delivery', must be one of #{validator.allowable_values}."
      end
      @non_delivery = non_delivery
    end
